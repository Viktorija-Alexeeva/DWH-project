CREATE OR REPLACE PROCEDURE BL_CL.load_ce_product_groups_init ()
LANGUAGE plpgsql
AS $$
DECLARE 
	diag_row_count INT;
...
SELECT -1, '-1', 'MANUAL', 'MANUAL', 'N/A',
		COALESCE(product_group_category_id, -1),
		to_timestamp(1900-01-01) ,
		to_timestamp(1900-01-01)  
FROM bl_3nf.ce_product_group_categories
WHERE product_group_category_srcid = '-1'
ON CONFLICT (product_group_srcid) DO NOTHING;

-- get diagnostics	
	GET DIAGNOSTICS diag_row_count = ROW_COUNT;
		CALL BL_CL.load_log_data('BL_3NF',     -- INSERT ROW into table log_data if no errors
								'ce_product_groups_def' ,
								diag_row_count, 
								flag);
EXCEPTION
    WHEN OTHERS THEN
