CREATE OR REPLACE PROCEDURE BL_CL.load_ce_address_cities_init ()
LANGUAGE plpgsql
AS $$
DECLARE 
	diag_row_count INT;
...
SELECT  -1, '-1', 'MANUAL', 'MANUAL', 'N/A',
		COALESCE(address_city_country_id, -1),
		to_timestamp(1900-01-01) ,
		to_timestamp(1900-01-01)  
FROM bl_3nf.ce_address_city_countries
WHERE address_city_country_srcid = '-1'
