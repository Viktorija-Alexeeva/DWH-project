CREATE OR REPLACE PROCEDURE BL_CL.load_ce_products_init ()
LANGUAGE plpgsql
AS $$
DECLARE 
	diag_row_count INT;
...
SELECT -1, '-1', 'MANUAL', 'MANUAL', 'N/A',
		COALESCE(product_group_id, -1), -99, 'N', 
		to_timestamp(1900-01-01) ,
		to_timestamp(1900-01-01)  
FROM bl_3nf.ce_product_groups
WHERE product_group_srcid = '-1'
ON CONFLICT ( product_srcid) DO NOTHING;
